...
class NewmanRunner:
    def __init__(self, collection_path, environment_path=None, report_dir=None, reporters=None):
        """
        ...
        self.report_dir = Path(report_dir) if report_dir else self.collection_path.parent
    ...
    def run_collection(self):
        ...
        command += ["--reporters", ",".join(self.reporters), "--reporter-json-export", str(self.report_file)]
        ...
        try:
            ...
            print("Error: Seems Newman is not installed, please install first and then try !")
    ...
    def summarize_results(self):
        ...
        if not self.result_json:
            raise RuntimeError("No results found. Please run the collection first using run_collection().")
        ...
        for exec_ in executions:
            ...
            if failed:
                ...
                summary["failed_requests"].append({
                    ...
                    "errors": [a.get("error") for a in assertion_results if a.get("error")]
                })
            else:
                summary["passed"] += 1
