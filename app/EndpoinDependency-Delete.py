        ...
        for path, methods in paths.items():
            for method, md in methods.items():
                ...
                for status, resp in (md.get("responses") or {}).items():
                    resp_obj = self._resolve_ref(resp.get("$ref")) if "$ref" in resp else resp
                    if not resp_obj:
                        continue
                    # collect links
    ...
    def infer(self):
        ...
        for i, prod_key in enumerate(endpoints):
            for cons_key in endpoints:
                ...
                if ev:
                    edge_key = (f"{prod_key[0]} {prod_key[1]}", f"{cons_key[0]} {cons_key[1]}")
    ...
    def export_html(self, edges, outpath):
        ...
        for n, data in G.nodes(data=True):
            ...
            try:
                ...
                prefix = path.strip("/").split("/")[0] if "/" in path else path.strip("/")
        ...
        for u, v, data in G.edges(data=True):
            net.add_edge(u, v, value=data.get("weight", 1), title=data.get("title"), arrows="to")
