...
def upload():
    ...
    if not allowed_file(file.filename):
        ...
        return jsonify({"error": "Unsupported file type. Only YAML or JSON allowed."}), 400
    ...
    if len(file_content) > app.config['MAX_CONTENT_LENGTH']:
        logger.error("Invalid file size, it should be less than 10 K  'Size = {content_size}' !")
    ...
    #new_upload = Upload(id=uuid_value,email=email, status=False,vdata=vdata, filename=opeanapi)
    #db.session.add(new_upload)
    #db.session.commit()
    return jsonify({
        "message": f"ACME has received your request and it will be processed shortly. Please check your email to download the generated test cases and environment variables.",
        "email": email
    }), 200
...
if __name__ == '__main__':
    app.run(debug=True, host='0.0.0.0', port=3001) # nosec
