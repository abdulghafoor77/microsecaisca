...
class OpenAPIHandler:
    ...
    def __init__(self, openapi_file: str):
        ...
        self._log_audit("initialize", f"Initialized OpenAPIHandler for file {self.abs_path}")
    ...
    def _load_file(self):
        ...
        if not os.path.exists(self.openapi_file):
            self._log_audit("load_file", f"File not found: {self.abs_path}", user="system")
        ...
        with open(self.openapi_file, "r", encoding="utf-8") as f:
            if ext in [".yaml", ".yml"]:
                ...
                self._log_audit("load_file", f"Unsupported file format: {ext}", user="system")
                raise ValueError(f"Unsupported file format: {ext}. Use .json or .yaml/.yml")

        self._log_audit("load_file", f"Successfully loaded OpenAPI file: {self.abs_path}")
    ...
    def get_endpoints(self, user: str = "system"):
        ...
        self._log_audit("access_endpoints",
