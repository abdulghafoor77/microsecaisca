...
def cmd_gt(args):
    print("=============================================================== 1")
    print(f"[GT] File: {args.file} | Vul File: {args.vul} | output Directory {args.output}")
    
    if not os.path.isfile(args.file):
        print(f"❌  No input OpenAPI file exists !")
        return
    
    if not os.path.isfile(args.vul):
        print(f"❌ No input Vulernability file exists !")
        return
...
def cmd_run(args):
    print("=============================================================== 2")
    print(f"[RUN] Testcase: {args.testcase} | Attributes: {args.attributes} | Output: {args.output}")
...
def build_cli():
    ...
    gt_parser.add_argument("-o", "--output", required=True, help="Output Testcases file")
    ...
    run_parser.add_argument("-a", "--attributes", required=True, help="Attributes JSON file")
...
def ret_env(jObj, fileDir):
    ...
    allVarVal = create_postman_environment_file("ACME Environment variables", unique_list)
    ...
    if os.path.exists(file_path):
        ...
        print("If you press Y, the existing file will be overwritten, and any environment variables (if already set) will be deleted!")
