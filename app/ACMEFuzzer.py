...
class ACMEFuzzer:
    ...
    def fuzz_number_values(self):
        ...
        return [
            ...
            random.randint(-999999999, 999999999), # nosec
            ...
            for method, details in methods.items():
                ...
                for _ in range(5):
                    ...
                    for param in params:
                        ...
                        if param["in"] == "path":
                            ...
                            request["url"]["query"].append({
                                "key": param["name"],
                                "value": str(fuzzed_val)
                            })

                    # Request body fuzzing
                    ops = method.upper()
                    if ops == 'PUT' or ops == 'POST' or ops == 'PATCH' or ops == 'DELETE':
                        if body_schema:
                            component = self.extract_req_component_schema(body_schema, details)
                    ...
                    for item in security:
                        if "bearerAuth" == item:
                            secheader =  [{"key": "Authorization","value": "Bearer {{fuzz_auth_token_valid}}","type": "text"}]
                            header.extend(secheader)

                        if "basicAuth" == item:
                            secheader =  [{"key": "Authorization","value": "Basic {{fuzz_auth_base64}}","type": "text"}]
                            header.extend(secheader)
                    request["header"] = header

                    fuzzed_cases.append({
                        "name": f"FUZZ - {ops} {path}",
                        "request": request
                    })
...
if __name__ == "__main__":
    openapi_file = "/home/chaincode/Desktop/acmeimp/microsecai/input/updatemedicalreport.yaml"
    fuzzer = ACMEFuzzer()
