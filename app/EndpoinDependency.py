...
crud_order = [
    "create", "add", "insert", "store", "register", "signup", "new", "init", "setup", "build", "provision",
    "read", "get", "find", "fetch", "list", "search", "query", "retrieve", "view", "details", "info", "summary",
    "update", "edit", "modify", "patch", "put", "change", "merge", "revise", "replace", "correct", "adjust",
    "delete", "remove", "destroy", "revoke", "cancel", "archive", "clear", "purge", "drop", "disable", "deactivate",
    "export", "export-csv", "export-all", "download", "download-csv", "report", "generate-report", "backup",
    "login", "logout", "auth", "authenticate", "token", "refresh", "verify-token", "session", "signin", "signout",
    "role", "permission", "access", "authorize", "grant", "revoke", "policy", "security",
    "approve", "reject", "activate", "deactivate", "submit", "process", "complete", "finalize", "start", "stop",
    "sanitize", "validate", "deserialize", "serialize", "encode", "decode", "normalize", "transform", "convert",
    "notify", "send", "message", "alert", "email", "sms", "push", "webhook", "broadcast", "signal",
    "check", "ping", "health", "status", "configuration", "settings", "config", "metrics", "logs", "history",
    "process-file", "process-data", "calculate", "compute", "aggregate", "analyze", "evaluate"
]
...
def extract_dependencies(endpoint):
    ...
    for media_type in req_body.values():
        ...
        if "$ref" in schema:
            ...
            dependencies.update([k.lower() for k in schema.get("properties", {}).keys() if k.lower() in dependency_keys])
        elif "properties" in schema:
            dependencies.update([k.lower() for k in schema["properties"].keys() if k.lower() in dependency_keys])
    return dependencies
...
def generate_execution_sequence(openapi_file):
    ...
    for ep in endpoints:
        ...
        if any("auth" in tag.lower() for tag in tags) or "login" in op_id or "login" in summary:
            ep["order"] = 0
        ...
        for ep in remaining:
            ...
            if not ep["dependencies"] or ep["dependencies"].issubset(produced_keys):
                ...
                for i, keyword in enumerate(crud_order):
                    if keyword in op_id or keyword in summary or keyword in ep["method"].lower():
                        ep["order"] = i + 1
                        break
                if ep["order"] is None:
    ...
    for i, ep in enumerate(execution_sequence, start=1):
        step_name = ep["details"].get("summary") or ep["details"].get("operationId") or f"{ep['method']} {ep['path']}"
        print(f"{i}. {step_name}")
