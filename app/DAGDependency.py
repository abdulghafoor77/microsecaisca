...
TOKEN_FIELD_NAMES = {"token", "access_token", "refresh_token", "id_token", "jwt", "auth_token", "sessionid", "session_id"}
...
class OpenApiExecPlanner:
    ...
    def detect_auth_producers_and_protected(self):
        ...
        for node in list(self.graph.nodes):
            ...
            hint_auth = any("auth" in t or "login" in t or "token" in t for t in tags) or "login" in opid or "login" in summary or "auth" in opid
            ...
            if op_security is None:
                if self.global_security:
                    ...
                    self.protected_headers[node] = self.get_required_headers(self.global_security)
            else:
                if isinstance(op_security, list) and len(op_security) > 0:
                    protected_nodes.add(node)
                    self.protected_headers[node] = self.get_required_headers(op_security)
    ...
    def run(self):
        ...
        print(f"Auth producers detected: {len(auth_producers)} Protected endpoints: {len(protected_nodes)}")
...
if __name__ == "__main__":
    ...
    out_json = "/home/chaincode/Desktop/acmeimp/microsecai/input/execution_sequence.json"
